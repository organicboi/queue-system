        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {branches.map((branch) => (
            <div key={branch.id} className="rounded-xl border border-border bg-white overflow-hidden">
              <Link href={`/branches/${branch.id}`} className="block p-5 active:bg-muted/20 transition-colors group">
                <div className="flex items-start justify-between">
                  <div className="size-9 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                    <GitBranch className="size-4 text-primary" />
                  </div>
                  <ChevronRight className="size-4 text-muted-foreground group-active:text-primary transition-colors mt-1" />
                </div>
                <div className="mt-3">
                  ...
                  <div className="mt-2 flex items-center gap-1.5">
                    <span className={`size-1.5 rounded-full ${branch.isActive ? 'bg-accent-600' : 'bg-slate-300'}`} />
              ...
              <div className="flex items-center gap-0 border-t border-border divide-x divide-border">
                <Link
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs text-muted-foreground active:text-slate-700 active:bg-muted/20 transition-colors"
                >
