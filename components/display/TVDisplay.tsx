...
export function TVDisplay({ theme, businessName, businessType, tickerText, branchId, announcementLang = 'en' }: Props) {
  ...
  const announce = useCallback((queueNumber: number) => {
    ...
    if ("AndroidTTS" in window) {
      const text = announcementLang === 'ar' ? arText
        : announcementLang === 'both' ? `${enText} ${arText}`
        : enText
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    >
      ...
      <AnimatePresence>
        {!audioReady && (
          ...
          >
            <p className="font-black text-white text-center" style={{ fontSize: "clamp(1.8rem, 4vw, 4rem)", letterSpacing: "-0.02em" }}>
              Tap anywhere to enable audio
            </p>
            <p className="mt-4 text-slate-400 text-center" style={{ fontSize: "clamp(0.9rem, 1.6vw, 1.4rem)" }}>
            ...
            <div
              ...
              style={{ fontSize: "clamp(0.6rem, 1vw, 0.9rem)", padding: "0.75em 2.5em", backgroundColor: theme.tickerChipBg, color: theme.tickerChipText }}
            >
              Enable Audio
            </div>
          </motion.div>
      ...
      <header
        ...
        style={{ height: "clamp(60px, 7vh, 88px)", backgroundColor: theme.navBg, borderBottom: `1px solid ${theme.navBorder}` }}
      >
        <div>
          <h1 className="font-black tracking-tight leading-none" style={{ fontSize: "clamp(1.1rem, 1.8vw, 1.9rem)", color: theme.navTitle }}>
            {businessName}
          </h1>
          <p className="uppercase tracking-widest font-semibold mt-1" style={{ fontSize: "clamp(0.5rem, 0.8vw, 0.75rem)", color: theme.navSub }}>
            {businessType} Queue
          </p>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT — 40% */}
        <div className="flex flex-col overflow-hidden" style={{ width: "40%", borderRight: `1px solid ${theme.sectionBorder}` }}>
          {/* NOW SERVING */}
          <div
          ...
          >
            ...
            <div className="flex flex-col items-center mt-4">
              ...
              {currentEntry ? (
                ...
                <p className="mt-5" style={{ fontSize: "clamp(0.8rem, 1.3vw, 1.1rem)", color: theme.servingEmpty }}>
                ...
                >
                  ...
                  {calledInfo.callCount > 1 && (
                    <motion.div
                      ...
                      style={{ backgroundColor: theme.callingRecallBg, border: `1px solid ${theme.callingRecallBorder}` }}
                    >
                      <span className="size-2 rounded-full animate-pulse" style={{ backgroundColor: theme.callingRecallDot }} />
                      <span className="font-semibold uppercase tracking-widest" style={{ fontSize: "clamp(0.6rem, 0.9vw, 0.8rem)", color: theme.callingRecallText }}>
          ...
          <div className="flex overflow-hidden" style={{ flex: 2 }}>
            ...
            >
              <p className="shrink-0 font-black uppercase tracking-[0.4em] mb-2" style={{ fontSize: "clamp(0.5rem, 0.8vw, 0.7rem)", color: theme.sectionLabel }}>
                Next in Line
              </p>
              <div className="flex-1 overflow-hidden flex flex-col gap-1.5">
                ...
                {nextWaiting.length === 0 && (
                  <p className="text-center py-6" style={{ fontSize: "clamp(0.7rem, 1vw, 0.9rem)", color: theme.emptyText }}>
                    Queue is empty
                  </p>
                )}
              </div>
            ...
            <div className="flex flex-1 flex-col overflow-hidden px-4 py-3" style={{ backgroundColor: theme.recentBg }}>
              <p className="shrink-0 font-black uppercase tracking-[0.4em] mb-2" style={{ fontSize: "clamp(0.5rem, 0.8vw, 0.7rem)", color: theme.sectionLabel }}>
                Recently Served
              </p>
              <div className="flex-1 overflow-hidden flex flex-col gap-1.5">
