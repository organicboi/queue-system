...
import {
  ...
  Pencil, UserX, UserCheck, CalendarPlus,
} from 'lucide-react'
import {
...
import {
  ...
  updateHospitalPatientAction,
  deactivateHospitalPatientAction,
  reactivateHospitalPatientAction,
  receptionIssueTokenAction,
  receptionCancelTokenAction,
  receptionRejoinTokenAction,
  bookHospitalAppointmentAction,
...
import {
  type ReceptionDept, type ReceptionDoc, PRIORITY_CATEGORIES, tomorrowOf, fromInstant, formatSlot, toSlotLocal,
} from '@/lib/hospital/receptionShared'
import type { HospitalPatientDetail } from '@/lib/dal/hospital'
import type { HospitalPatientDTO, HospitalAppointmentDTO, HospitalTokenDTO } from '@/lib/db/hospital-types'
              ...
              {results.map((p) => (
                <li key={p.id}>
                  ...
                  >
                    ...
                    {!p.isActive && (
                      <span className="ms-1.5 rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500">
                        Inactive
                      </span>
                    )}
...
function Field({ name, label, required, type = 'text', placeholder, defaultValue }: {
  name: string; label: string; required?: boolean; type?: string; placeholder?: string; defaultValue?: string
}) {
  return (
    <div className="space-y-1.5">
      <Label htmlFor={name}>{label}</Label>
      <Input id={name} name={name} required={required} type={type} placeholder={placeholder} defaultValue={defaultValue} />
    </div>
  )
}
...
function PatientDetail({ branchId, serviceDate, detail, departments, doctors, onChange }: {
  ...
  const { patient, visits, appointments, tokens } = detail
  ...
  const [reschedule, setReschedule] = useState<{ appt: HospitalAppointmentDTO; token: HospitalTokenDTO | undefined } | null>(null)
  ...
  const tokenForAppt = (apptId: string) => tokens.find((t) => t.appointmentId === apptId)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="flex items-center gap-1.5 text-lg font-bold text-slate-900">
            {patient.name}
            {!patient.isActive && (
              <span className="rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500">Inactive</span>
            )}
          </p>
        ...
        <div className="flex shrink-0 gap-1.5">
          ...
          {patient.isActive ? (
            <button
              onClick={() => amend(() => deactivateHospitalPatientAction(branchId, patient.id), 'Patient deactivated')}
              disabled={pending}
            ...
            <button
              onClick={() => amend(() => reactivateHospitalPatientAction(branchId, patient.id), 'Patient reactivated')}
              disabled={pending}
              className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 px-2 py-1 text-[11px] font-medium text-emerald-600"
            >
      ...
      <section className="rounded-xl border border-slate-200 p-3">
        ...
        <Button onClick={issue} disabled={pending || !patient.isActive} className="mt-2 w-full bg-accent-600 hover:bg-accent-700 text-white">
          Issue token
        </Button>
      </section>
      ...
      {/* Schedule an appointment (advance token — the day's series carries it before the kiosk opens) */}
      <section className="rounded-xl border border-slate-200 p-3">
        <p className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-slate-800">
          <CalendarPlus className="size-4" /> Schedule an appointment
        </p>
        <HospitalAppointmentForm
          departments={departments}
          doctors={doctors}
          minDate={serviceDate}
          defaults={{ date: tomorrowOf(serviceDate) }}
          submitLabel="Book"
          pending={pending}
          onSubmit={(v) => startTransition(async () => {
            if (!patient.isActive) { toast.error('This patient is inactive — reactivate first'); return }
            const res = await bookHospitalAppointmentAction({
              branchId, patientId: patient.id, departmentId: v.departmentId, doctorId: v.doctorId,
              slotLocal: toSlotLocal(v.date, v.time), priorityCategory: v.priority || null,
            })
            if (res.error) toast.error(res.error)
            else { toast.success(`Booked ${res.token?.tokenCode} for ${v.date}`); onChange() }
          })}
        />
      </section>
      ...
      {activeAppointments.length > 0 && (
        <section>
          <p className="mb-1.5 text-xs font-bold uppercase tracking-wide text-slate-400">Appointments</p>
          <ul className="space-y-1.5">
            {activeAppointments.map((a) => {
              const t = tokenForAppt(a.id)
              return (
                <li key={a.id} className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm">
                  {t && <span className="font-mono font-bold tabular-nums text-slate-800" dir="ltr">{t.tokenCode}</span>}
                  <span className="text-slate-700">{formatSlot(a.slotTime)}</span>
                  <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500">
                    {t ? (STAGE_LABEL[t.stage] ?? t.status) : a.status}
                  </span>
                  {t && t.status !== 'served' && t.status !== 'cancelled' && (
                    <span className="ms-auto flex gap-1.5">
                      <button
                        onClick={() => setReschedule({ appt: a, token: t })}
                        className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2 py-1 text-[11px] font-medium text-slate-600"
                      >
                        <Pencil className="size-3" /> Reschedule
                      </button>
                      <button
                        onClick={() => amend(() => cancelHospitalAppointmentAction(branchId, a.id), `${t.tokenCode} cancelled`)}
                        disabled={pending}
                        className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-2 py-1 text-[11px] font-medium text-red-600"
                      >
                        <Ban className="size-3" /> Cancel
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <EditPatientForm branchId={branchId} patient={patient} onDone={() => { setEditOpen(false); onChange() }} />
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!reschedule} onOpenChange={(open) => !open && setReschedule(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reschedule {reschedule?.token?.tokenCode}</DialogTitle>
          </DialogHeader>
          {reschedule && reschedule.token && (
            <HospitalAppointmentForm
              ...
              minDate={serviceDate}
              defaults={{
                departmentId: reschedule.token.departmentId, doctorId: reschedule.appt.doctorId,
                ...fromInstant(reschedule.appt.slotTime), priority: reschedule.token.priorityCategory ?? '',
              }}
              submitLabel="Save"
              pending={pending}
              ...
              onSubmit={(v) => startTransition(async () => {
                const res = await rescheduleHospitalAppointmentAction({
                  branchId, appointmentId: reschedule.appt.id, departmentId: v.departmentId, doctorId: v.doctorId,
                  slotLocal: toSlotLocal(v.date, v.time), priorityCategory: v.priority || null,
                })
                if (res.error) toast.error(res.error)
                else { toast.success(`Rescheduled to ${res.token?.tokenCode}`); setReschedule(null); onChange() }
              })}
            />
          )}
        </DialogContent>
...
const EDIT_INIT: { patient?: HospitalPatientDTO; error?: string } = {}

function EditPatientForm({ branchId, patient, onDone }: {
  branchId: string; patient: HospitalPatientDTO; onDone: () => void
}) {
  const [state, formAction, pending] = useActionState(updateHospitalPatientAction, EDIT_INIT)

  useEffect(() => {
    if (state.patient) onDone()
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      <Field name="name" label="Full name" required defaultValue={patient.name} />
      <div className="grid grid-cols-2 gap-3">
        <Field name="phone" label="Phone" type="tel" defaultValue={patient.phone} />
        <Field name="uhid" label="UHID" required defaultValue={patient.uhid} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Field name="dob" label="Date of birth" type="date" defaultValue={patient.dob ?? ''} />
        <div className="space-y-1.5">
          <Label htmlFor="gender">Gender</Label>
          <select id="gender" name="gender" defaultValue={patient.gender ?? ''} className="h-9 w-full rounded-lg border border-border bg-white px-2 text-sm">
            <option value="">—</option>
            <option value="male">Male</option>
            <option value="female">Female</option>
            <option value="other">Other</option>
      ...
      <Field name="abhaNumber" label="ABHA number (optional)" defaultValue={patient.abhaNumber ?? ''} />
      {state.error && <p className="text-sm text-red-600">{state.error}</p>}
      <Button type="submit" disabled={pending} className="w-full bg-accent-600 hover:bg-accent-700 text-white">
        {pending ? 'Saving…' : 'Save'}
      </Button>
    </form>
  )
